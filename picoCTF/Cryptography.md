...
But first we need to fully understand the code, the code has different functions that do ifferent things the main one is `test` it starts the encryption process. It first takes in values of "p" and "g" and checks if they are prime or not using `is_prime`, if they arent prime it retakes their values here in our case the values are "p=97" and "g=31" both are prime. Now it moves on to key generation for which it randomly generates "a" and "b" (luckily we know the value of them used for encoding) and uses them to generate "u" and "v" using `generator` (which basically takes in three parameters and puts them in the formula (g^x)%p and returns the value), so we now have values of "u" and "v" which are used to generate the "key" and "b_key" again by using `generator` and if both have equal values then "shared_key" is assigned the value of "key". Now all the keys are generated and it starts the encoding by first getting a semi ciphertext by using the `dynamic_xor_encrypt` which takes in the "flag" and a "text_key" then it reverses the "flag" and XOR's it with the "text_key" to get the semi cipher. Finally it takes the semi cipher and gives it to `encrypt` along with the "shared_key" which basically takes each character from semi cipher converts it to ASCII then multiplies it with the "shared_key" and 311 then appends the number to a list "cipher" which is the final ciphertext.   
